...
class QClass:
    def __init__(self,
        ...
        self._q_shape = self._environment.states_shape + self._environment.actions_shape
        self._q_values: np.ndarray = np.empty(shape=self._q_shape, dtype=float)
        ...
        for state_ in self._environment.states():
            for action_ in self._environment.actions_for_state(state_):
                ...
                if state_.is_terminal:
                    self._q_values[q_index] = 0.0
                else:
                    self._q_values[q_index] = constants.INITIAL_Q_VALUE
    ...
    def state_argmax(self, state: environment.State) -> environment.Action:
        ...
        q_state: np.ndarray = self._q_values[q_slice]
